...
public OnPluginStart()
{
	...
	cv_walltex = CreateConVar("sm_wk_text", "effects/energyball.vmt", "Wallhack Texture", FCVAR_PLUGIN|FCVAR_NOTIFY);
...
public Action:Command_Wallhack(client, args)
{
	if(CheckCommandAccess(client, "sm_wallhack_access", ADMFLAG_ROOT))
	{
		if(GetClientTeam(client) == 1)
		{
			...
			{
				Walling[client] = true;
				PrintToChat(client, "WH Enabled");
			}
			else if(Walling[client] == true)
			{
				Walling[client] = false;
				PrintToChat(client, "WH Disabled");
			}
		}
