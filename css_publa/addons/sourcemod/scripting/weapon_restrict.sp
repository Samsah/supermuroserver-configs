...
RegisterHacks()
{
	...
	if(FindConVar("cssdm_version") == INVALID_HANDLE)//Check if dm is loaded at all if it is we cant register weapon drop or we'll crash the server.
	{
	...
	{
		StartPrepSDKCall(SDKCall_Player);
		PrepSDKCall_SetFromConf(gameConf, SDKConf_Virtual, "Weapon_Drop");
		PrepSDKCall_AddParameter(SDKType_CBaseEntity, SDKPass_Pointer);
