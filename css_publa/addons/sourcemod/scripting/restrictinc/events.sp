...
public Action:RemoveEdictDelay(Handle:timer, any:index)
{
...
{
	if(FindConVar("cssdm_version") != INVALID_HANDLE)//Deathmatch mode version cvar is set via extension so if it exists the extension is loaded most likely better to check every time just incase of late load
	{
		if(GetPlayerWeaponSlot(client, slot))//Just incase make sure it didnt disappear
		{
			new Float:orgin[3] = {-10000.0, -10000.0, -10000.0};
			SDKCall(weaponDrop, client, weapon, orgin, orgin);
			CreateTimer(0.1, RemoveEdictDelay, weapon, TIMER_FLAG_NO_MAPCHANGE);//No delay = crash
		}
	}
	else
	{
