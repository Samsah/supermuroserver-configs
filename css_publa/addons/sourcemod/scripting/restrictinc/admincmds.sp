...
{
	if(FindConVar("cssdm_version") != INVALID_HANDLE)//Drop c4 dosnt work if dm is on cause we'll crash the server
	{
		ReplyToCommand(client, "Deathmatch is active cant drop bomb");
		return Plugin_Handled;
	}
	new bomb;
