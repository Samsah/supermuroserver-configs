...
new String:modelCT[7][128];
new String:modelT[7][128];
...
public OnMapStart()
{
	...
	PrecacheModel("models/player/ctm_fbi.mdl",true);
	PrecacheModel("models/player/ctm_gign.mdl",true);
	PrecacheModel("models/player/ctm_gsg9.mdl",true);
	PrecacheModel("models/player/ctm_idf.mdl",true);
	PrecacheModel("models/player/ctm_sas.mdl",true);
	PrecacheModel("models/player/ctm_st6.mdl",true);
	PrecacheModel("models/player/ctm_swat.mdl",true);

	PrecacheModel("models/player/tm_anarchist.mdl",true);
	PrecacheModel("models/player/tm_phoenix.mdl",true);
	PrecacheModel("models/player/tm_pirate.mdl",true);
	PrecacheModel("models/player/tm_professional.mdl",true);
	PrecacheModel("models/player/tm_separatist.mdl",true);

	modelCT[0] = "models/player/ctm_fbi.mdl";
	modelCT[1] = "models/player/ctm_gign.mdl";
	modelCT[2] = "models/player/ctm_gsg9.mdl";
	modelCT[3] = "models/player/ctm_idf.mdl";
	modelCT[4] = "models/player/ctm_sas.mdl";
	modelCT[5] = "models/player/ctm_st6.mdl";
	modelCT[6] = "models/player/ctm_swat.mdl";

	modelT[0] = "models/player/tm_anarchist.mdl";
	modelT[1] = "models/player/tm_phoenix.mdl";
	modelT[2] = "models/player/tm_pirate.mdl";
	modelT[3] = "models/player/tm_professional.mdl";
	modelT[4] = "models/player/tm_separatist.mdl";
}
	...
	if (GetConVarBool(astEnable))
	{
		...
		if (GetConVarInt(astType) == 5)
		{
			...
			if (nRound == maxRound)
			{
				...
				{
					CPrintToChatAll("\x01[\x04Muro\x01]\x04 %t","advert_will_swap");
				}
			...
			else
			{
				if (GetConVarBool(astAdvertRound))
				{
					...
					CPrintToChatAll("\x01[\x04Muro\x01]\x04 %t","advert_rounds",difference+1);
				}
			...
			if (!isSwap)
			{
					...
					if (GetMapTimeLimit(timeLimit) && timeLimit > 0)
					{
						...
						if (GetMapTimeLeft(timeLeft) && timeLeft > 0)
						{
							...
							else if (GetConVarBool(astAdvertRound))
							{
								...
								CPrintToChatAll("\x01[\x04Muro\x01]\x04 %t","advert_time",difference);
				...
				else if (GetConVarInt(astType) == 3)
				{
					...
					if (winLimit > 0)
					{
						...
						if (GetConVarBool(astAdvertRound))
						{
							CPrintToChatAll("\x01[\x04Muro\x01]\x04 %t","advert_wins",winSwap);
						}
				...
				else if (GetConVarInt(astType) == 4)
				{
					...
					if (fragLimit > 0)
					{
						...
						if (GetConVarBool(astAdvertRound))
						{
							CPrintToChatAll("\x01[\x04Muro\x01]\x04 %t","advert_frags",fragSwap);
						}
...
public Action:AutoSwap(Handle:timer)
{
	...
	{
		CPrintToChatAll("\x01[\x04Muro\x01]\x04 %t","advert_were_swap");
	}
