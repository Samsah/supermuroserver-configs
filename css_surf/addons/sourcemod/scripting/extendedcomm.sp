/*
	...
	Revision 3.0.5:
		Fixed a rare case where a player who is permanently gagged/muted and has an extended punishment expire would have parts of the punishment erased.
		Clients are properly flagged for saving if their extended punishment expires while the client is not in the server.
			- Also provides support for issuing save queries immediately if delay queries is disabled.
		Reworked a bit of the update logic to attempt to narrow down random errors.
		Increased the allowed string size for various elements within the script to increase compatibility.
		Added some extra code to ensure data is cleared on new clients to prevent old punishments from carrying over.
*/
...
public OnClientConnected(client)
{
	if(client > 0 && !IsFakeClient(client))
	{
		...
		g_Immunity[client] = 0;
		g_sName[client][0] = g_sSteam[client][0] = '\0';
	}
}
...
public Action:Command_Prune(client, args) 
{
	...
	else
	{
		decl String:_sQuery[512];
		Format(_sQuery, sizeof(_sQuery), g_sSQL_PruneSelect, COMM_TABLE);
		SQL_TQuery(g_hDatabase, SQL_AutoPrune, _sQuery, GetClientUserId(client));
	}
	...
	else if(SQL_HasResultSet(hndl))
	{
		...
		else
		{
			if(SQL_FetchRow(hndl))
			{
				...
				else if(_iCurrent < VERSION_CURRENT_UPDATE)
				{
					switch(_iCurrent)
					{
						case 0:
							...
							else
							{
								switch(g_iUpdateSteps)
								{
									...
									case 1:
									{
										...
										SQL_TQuery(g_hDatabase, SQL_PerformUpdate_3_0_0, _sQuery, data);
									}
								}
							}
						}
	...
	else
	{
		...
		if(client > 0 && IsClientInGame(client))
		{
			if(SQL_HasResultSet(hndl))
			{
				if(SQL_FetchRow(hndl))
				{
					new _iRemaining, _iEnding, _iTime = GetTime(), bool:_bSave = false;
					...
					g_iGagState[client] = SQL_FetchInt(hndl, 6);
					g_iGagLength[client] = SQL_FetchInt(hndl, 7);
					g_iGagTime[client] = SQL_FetchInt(hndl, 8);
					SQL_FetchString(hndl, 9, g_sGagAdmin[client], 32);
					SQL_FetchString(hndl, 10, g_sGagReason[client], 64);
				...
				{
					g_iMuteLevel[client] = g_iMuteState[client] = g_iMuteLength[client] = g_iMuteTime[client] = 0;
					g_sMuteAdmin[client][0] = g_sMuteReason[client][0] = '\0';
					g_iGagLevel[client] = g_iGagState[client] = g_iGagLength[client] = g_iGagTime[client] = 0;
					g_sGagAdmin[client][0] = g_sGagReason[client][0] = '\0';
				}
			}
		}
