/*
    SourceMod Anti-Cheat ConVars Module
...
CVars_OnPluginStart()
{
	...
	g_hCVarCVarsEnabled = CreateConVar("smac_cvars_enable", "1", "Enable the CVar checks module.", FCVAR_PLUGIN);
	...
	do
	{
		...
		if ((f_iFlags & FCVAR_REPLICATED) != FCVAR_REPLICATED)
			continue;
		...
		if ((f_iFlags & FCVAR_PLUGIN) == FCVAR_PLUGIN)
			continue;
...
public Action:CVars_CmdAddCVar(client, args)
{
	...
	{
		ReplyToCommand(client, "Unrecognized comparison type \"%s\", acceptable values: \"equal\", \"greater\", \"less\", \"between\", or \"strequal\".", f_sTemp);
		return Plugin_Handled;
	}
	...
	if ( CVars_AddCVar(f_sCVarName, f_iCompType, f_iAction, f_sValue, f_fValue2, PRIORITY_NORMAL) )
	{
		if ( client )
		{
			...
			SMAC_Log("%N (ID: %s | IP: %s) added convar %s to the check list.", client, f_sAuthID, f_sIP, f_sCVarName);
		}
...
public Action:CVars_CmdRemCVar(client, args)
{
	...
	if ( CVars_RemoveCVar(f_sCVarName) )
	{
		if ( client )
		{
			GetClientAuthString(client, f_sAuthID, sizeof(f_sAuthID));
			GetClientIP(client, f_sIP, sizeof(f_sIP));
			SMAC_Log("%N (ID: %s | IP: %s) removed convar %s from the check list.", client, f_sAuthID, f_sIP, f_sCVarName);
		}
		else
...
public CVars_QueryCallback(QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName[], const String:cvarValue[])
{
	...
	decl String:f_sCVarName[64], Handle:f_hConVar, Handle:f_hTemp, String:f_sName[MAX_NAME_LENGTH], String:f_sAuthID[64], String:f_sIP[64], f_iCompType, f_iAction, String:f_sValue[64], Float:f_fValue2, String:f_sAlternative[128], f_iSize, bool:f_bContinue;
	...
	if ( !StrEqual(cvarName, f_sCVarName) ) // CVar not expected.
	{
		...
		{
			SMAC_Log("Unknown CVar Reply: %N (ID: %s | IP: %s) was kicked for a corrupted return with convar name \"%s\" (expecting \"%s\") with value \"%s\".", client, f_sAuthID, f_sIP, cvarName, f_sCVarName, cvarValue);
	...
	if ( f_iCompType == COMP_NONEXIST )
	{
		...
		{
			SMAC_PrintToChatAdmins("%t", SMAC_HASPLUGIN, f_sName, f_sAuthID, f_sCVarName);
			...
			switch(f_iAction)
			{
				...
				case ACTION_MUTE:
				{
					CPrintToChatAll("%t", SMAC_MUTED, f_sName);
					ServerCommand("sm_mute #%d", GetClientUserId(client));
				}
				case ACTION_KICK:
				{
					SMAC_Log("Plugin CVar return: %N (ID: %s | IP: %s) was kicked for returning with plugin convar \"%s\" (value \"%s\", return %s).", client, f_sAuthID, f_sIP, cvarName, cvarValue, g_sQueryResult[result]);
					KickClient(client, "%t", SMAC_REMOVEPLUGINS);
					return;
				}
				case ACTION_BAN:
				{
					SMAC_Log("Bad CVar return: %N (ID: %s | IP: %s) has convar \"%s\" (value \"%s\", return %s) when it shouldn't exist.", client, f_sAuthID, f_sIP, cvarName, cvarValue, g_sQueryResult[result]);
					SMAC_Ban(client, g_iBanDuration, SMAC_BANNED, "SMAC: ConVar %s violation", cvarName);
	...
	{
		SMAC_Log("Bad CVar Query Result: %N (ID: %s | IP: %s) returned query result \"%s\" (expected Okay) on convar \"%s\" (value \"%s\").", client, f_sAuthID, f_sIP, g_sQueryResult[result], cvarName, cvarValue);
		SMAC_Ban(client, g_iBanDuration, SMAC_BANNED, "SMAC: %s violation (bad query result).", cvarName);
	...
	if ( f_iCompType != COMP_STRING )
	{
		...
		for(new i=0;i<f_iSize;i++)
		{
			...
			{
				SMAC_Log("Corrupted CVar response: %N (ID: %s | IP: %s) was kicked for returning a corrupted value on %s (%s), value set at \"%s\" (expected \"%s\").", client, f_sAuthID, f_sIP, f_sCVarName, cvarName, cvarValue, f_sValue);
				KickClient(client, "%t", SMAC_CLIENTCORRUPT);
				return;
			}
		}
	...
	switch(f_iCompType)
	{
		case COMP_EQUAL:
			...
			{
				SMAC_PrintToChatAdmins("%t", SMAC_HASNOTEQUAL, f_sName, f_sAuthID, f_sCVarName, cvarValue, f_sValue);
				...
				switch(f_iAction)
				{
					...
					case ACTION_KICK:
					{
						SMAC_Log("Bad CVar response: %N (ID: %s | IP: %s) was kicked for returning with convar \"%s\" set to value \"%s\" when it should be \"%s\".", client, f_sAuthID, f_sIP, cvarName, cvarValue, f_sValue);
						KickClient(client, "%t", SMAC_SHOULDEQUAL, cvarName, f_sValue, cvarValue);
						return;
					}
					case ACTION_BAN:
					{
						SMAC_Log("Bad CVar response: %N (ID: %s | IP: %s) has convar \"%s\" set to value \"%s\" (should be \"%s\") when it should equal.", client, f_sAuthID, f_sIP, cvarName, cvarValue, f_sValue);
						SMAC_Ban(client, g_iBanDuration, SMAC_BANNED, "SMAC: ConVar %s violation", cvarName);
						return;
					}
				}
		...
		case COMP_GREATER:
			if ( StringToFloat(f_sValue) > StringToFloat(cvarValue) )
			{
				SMAC_PrintToChatAdmins("%t", SMAC_HASNOTGREATER, f_sName, f_sAuthID, f_sCVarName, cvarValue, f_sValue);
				...
				switch(f_iAction)
				{
					...
					case ACTION_KICK:
					{
						SMAC_Log("Bad CVar response: %N (ID: %s | IP: %s) was kicked for returning with convar \"%s\" set to value \"%s\" when it should be greater than or equal to \"%s\".", client, f_sAuthID, f_sIP, cvarName, cvarValue, f_sValue);
						KickClient(client, "%t", SMAC_SHOULDGREATER, cvarName, f_sValue, cvarValue);
						return;
					}
					case ACTION_BAN:
					{
						SMAC_Log("Bad CVar response: %N (ID: %s | IP: %s) has convar \"%s\" set to value \"%s\" (should be \"%s\") when it should greater than or equal to.", client, f_sAuthID, f_sIP, cvarName, cvarValue, f_sValue);
						SMAC_Ban(client, g_iBanDuration, SMAC_BANNED, "SMAC: ConVar %s violation", cvarName);
						return;
					}
				}
		...
		case COMP_LESS:
			if ( StringToFloat(f_sValue) < StringToFloat(cvarValue) )
			{
				SMAC_PrintToChatAdmins("%t", SMAC_HASNOTLESS, f_sName, f_sAuthID, f_sCVarName, cvarValue, f_sValue);
				...
				switch(f_iAction)
				{
					...
					case ACTION_KICK:
					{
						SMAC_Log("Bad CVar response: %N (ID: %s | IP: %s) was kicked for returning with convar \"%s\" set to value \"%s\" when it should be less than or equal to \"%s\".", client, f_sAuthID, f_sIP, cvarName, cvarValue, f_sValue);
						KickClient(client, "%t", SMAC_SHOULDLESS, cvarName, f_sValue, cvarValue);
						return;
					}
					case ACTION_BAN:
					{
						SMAC_Log("Bad CVar response: %N (ID: %s | IP: %s) has convar \"%s\" set to value \"%s\" (should be \"%s\") when it should be less than or equal to.", client, f_sAuthID, f_sIP, cvarName, cvarValue, f_sValue);
						SMAC_Ban(client, g_iBanDuration, SMAC_BANNED, "SMAC: ConVar %s violation", cvarName);
						return;
					}
				}
		...
		case COMP_BOUND:
			...
			{
				SMAC_PrintToChatAdmins("%t", SMAC_HASNOTBOUND, f_sName, f_sAuthID, f_sCVarName, cvarValue, f_sValue, f_fValue2);
				...
				switch(f_iAction)
				{
					...
					case ACTION_KICK:
					{
						SMAC_Log("Bad CVar response: %N (ID: %s | IP: %s) was kicked for returning with convar \"%s\" set to value \"%s\" when it should be between \"%s\" and \"%f\".", client, f_sAuthID, f_sIP, cvarName, cvarValue, f_sValue, f_fValue2);
						KickClient(client, "%t", SMAC_SHOULDBOUND, cvarName, f_sValue, f_fValue2, cvarValue);
						return;
					}
					case ACTION_BAN:
					{
						SMAC_Log("Bad CVar response: %N (ID: %s | IP: %s) has convar \"%s\" set to value \"%s\" when it should be between \"%s\" and \"%f\".", client, f_sAuthID, f_sIP, cvarName, cvarValue, f_sValue, f_fValue2);
						SMAC_Ban(client, g_iBanDuration, SMAC_BANNED, "SMAC: ConVar %s violation", cvarName);
						return;
					}
				}
