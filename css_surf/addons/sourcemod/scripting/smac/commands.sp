/*
    SourceMod Anti-Cheat Commands Module
...
{
	g_hCVarCmdEnable = CreateConVar("smac_cmds_enable", "1", "If the Commands Module of SMAC is enabled.", FCVAR_PLUGIN);
	g_bCmdEnabled = GetConVarBool(g_hCVarCmdEnable);

	g_hCVarCmdSpam = CreateConVar("smac_cmds_spam", "30", "Amount of commands in one second before kick.  0 for disable.", FCVAR_PLUGIN);
	g_iCmdSpam = GetConVarInt(g_hCVarCmdSpam);

	g_hCVarCmdLog = CreateConVar("smac_cmds_log", "0", "Log command usage.  Use only for debugging purposes.", FCVAR_PLUGIN);
...
//- Setup logging path -//
	for(new i=0;;i++)
	{
		BuildPath(Path_SM, g_sCmdLogPath, sizeof(g_sCmdLogPath), "logs/SMACCmdLog_%d.log", i);
...
public Action:Commands_EventDisconnect(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ( f_iLength > 235 )
	{
		...
		if ( client )
		{
			...
			SMAC_Log("%L<%s> submitted a bad disconnect and was banned.", client, f_sIP);
			BanIdentity(f_sIP, 10080, BANFLAG_IP, "SMAC: Disconnect exploit."); // Prevent them from rejoining.
			if ( GetClientAuthString(client, f_sTemp, sizeof(f_sTemp)) )
			{
				if ( SOURCEBANS_AVAILABLE() ) // Prevent them from ever playing on these servers again.
					ServerCommand("sm_addban 0 \"%s\" \"SMAC: Disconnect exploit.\"", f_sTemp);
				else
					BanIdentity(f_sTemp, 0, BANFLAG_AUTHID, "SMAC: Disconnect exploit.", "SMAC");
			}
		}
	...
	for(new i=0;i<f_iLength;i++)
	{
		if ( f_sReason[i] < 32 )
		{
			if ( f_sReason[i] != '\n' )
			{
				SMAC_Log("Bad disconnect reason, \"%s\" len = %d", f_sReason, f_iLength);
				if ( client )
				{
					GetClientIP(client, f_sIP, sizeof(f_sIP));
					SMAC_Log("%L<%s> submitted a bad disconnect.  Possible corruption or attack.", client, f_sIP);
				}
				SetEventString(event, "reason", "Bad disconnect message");
				return Plugin_Continue;
			}
...
public Action:Commands_BlockExploit(client, const String:command[], args)
{
	if ( args > 0 )
	{
		...
		if ( StrEqual(f_sArg, "rcon_password") )
		{
			...
			SMAC_PrintToChatAdmins("%N was banned for command usage violation of command: sm_menu %s", client, f_sCmdString);
			SMAC_Log("%N (ID: %s | IP: %s) was banned for command usage violation of command: sm_menu %s", client, f_sAuthID, f_sIP, f_sCmdString);
			SMAC_Ban(client, g_iBanDuration, SMAC_CBANNED, "SMAC: Exploit violation");
			return Plugin_Stop;
		}
	}
	return Plugin_Continue;
...
public Action:Commands_CommandListener(client, const String:command[], argc)
{
	...
	if ( g_iCmdSpam != 0 && !GetTrieValue(g_hIgnoredCmds, f_sCmd, f_bBan) && ( StrContains(f_sCmd, "es_") == -1 || StrEqual(f_sCmd, "es_version") ) && g_iCmdCount[client]++ > g_iCmdSpam )
	{
		...
		GetClientIP(client, f_sIP, sizeof(f_sIP));
		GetCmdArgString(f_sCmdString, sizeof(f_sCmdString));
		SMAC_PrintToChatAdmins("%N was kicked for command spamming: %s %s", client, command, f_sCmdString);
		SMAC_Log("%N (ID: %s | IP: %s) was kicked for command spamming: %s %s", client, f_sAuthID, f_sIP, command, f_sCmdString);
		KickClient(client, "%t", SMAC_KCMDSPAM);
		return Plugin_Stop;
	}
	...
	if ( GetTrieValue(g_hBlockedCmds, f_sCmd, f_bBan) )
	{
		if ( f_bBan )
		{
			...
			GetClientIP(client, f_sIP, sizeof(f_sIP));
			GetCmdArgString(f_sCmdString, sizeof(f_sCmdString));
			SMAC_PrintToChatAdmins("%N was banned for command usage violation of command: %s %s", client, command, f_sCmdString);
			SMAC_Log("%N (ID: %s | IP: %s) was banned for command usage violation of command: %s %s", client, f_sAuthID, f_sIP, command, f_sCmdString);
			SMAC_Ban(client, g_iBanDuration, SMAC_CBANNED, "SMAC: Command %s violation", command);
		}
		return Plugin_Stop;
	}
...
public Action:Commands_ClientCheck(client, args)
{
	...
	if ( GetTrieValue(g_hBlockedCmds, f_sCmd, f_bBan) )
	{
		if ( f_bBan )
		{
			...
			GetClientIP(client, f_sIP, sizeof(f_sIP));
			GetCmdArgString(f_sCmdString, sizeof(f_sCmdString));
			SMAC_PrintToChatAdmins("%N was banned for command usage violation of command: %s %s", client, f_sCmd, f_sCmdString);
			SMAC_Log("%N (ID: %s | IP: %s) was banned for command usage violation of command: %s %s", client, f_sAuthID, f_sIP, f_sCmd, f_sCmdString);
			SMAC_Ban(client, g_iBanDuration, SMAC_CBANNED, "SMAC: Command %s violation", f_sCmd);
		}
		return Plugin_Stop;
	}
...
public Action:Commands_SpamCheck(client, args)
{
	...
	if ( g_iCmdSpam != 0 && !GetTrieValue(g_hIgnoredCmds, f_sCmd, f_bBan) && g_iCmdCount[client]++ > g_iCmdSpam )
	{
		...
		GetClientIP(client, f_sIP, sizeof(f_sIP));
		GetCmdArgString(f_sCmdString, sizeof(f_sCmdString));
		SMAC_PrintToChatAdmins("%N was kicked for command spamming: %s %s", client, f_sCmd, f_sCmdString);
		SMAC_Log("%N (ID: %s | IP: %s) was kicked for command spamming: %s %s", client, f_sAuthID, f_sIP, f_sCmd, f_sCmdString);
		KickClient(client, "%t", SMAC_KCMDSPAM);
		return Plugin_Stop;
	}
