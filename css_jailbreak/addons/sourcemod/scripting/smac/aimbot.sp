...
Aimbot_OnClientDisconnect(client)
{
	if (g_bAimbotEnabled)
	{
...
Aimbot_OnPlayerRunCmd(client, Float:angles[3])
{
	...
	g_fAimAngles[client][g_iAryIdx[client]][0] = angles[0];
	g_fAimAngles[client][g_iAryIdx[client]][1] = angles[1];
	g_fAimAngles[client][g_iAryIdx[client]][2] = angles[2];
...
Aimbot_AnalyzeAngles(client)
{
	...
	new count, idx = g_iAryIdx[client];
	
	while (count < AIM_RECORD_SIZE)
	{
		...
		if (idx == g_iAryIdx[client])
		{
			vLastAngles[0] = g_fAimAngles[client][idx][0];
			vLastAngles[1] = g_fAimAngles[client][idx][1];
			vLastAngles[2] = g_fAimAngles[client][idx][2];
			idx++;
			count++;
			continue;
		}
		
		vAngles[0] = g_fAimAngles[client][idx][0];
		vAngles[1] = g_fAimAngles[client][idx][1];
		vAngles[2] = g_fAimAngles[client][idx][2];
		...
		vLastAngles[0] = vAngles[0];
		vLastAngles[1] = vAngles[1];
		vLastAngles[2] = vAngles[2];
		idx++;
		count++;
	}
}
...
stock bool:IsVectorNull(Float:vec[3])
{
