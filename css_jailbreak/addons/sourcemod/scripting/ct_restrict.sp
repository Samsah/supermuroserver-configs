...
new stock const String:SQLQueries[SQLMAX__][E_QMax][] =
{
	...
	{ // MySQL
		"CREATE TABLE IF NOT EXISTS ct_restrict ( id INTEGER PRIMARY KEY NOT NULL AUTO_INCREMENT, name VARCHAR(65), steam VARCHAR(32) UNIQUE, ban_time DATE, admin_steam VARCHAR(32) );",
		"INSERT IGNORE INTO ct_restrict (name, steam, ban_time, admin_steam) VALUES ('%s', '%s', CURDATE(), '%s');",
		"SELECT id FROM ct_restrict WHERE steam = '%s';",
		"DELETE FROM ct_restrict WHERE id = '%i';",
		"UPDATE ct_restrict SET name = '%s' WHERE id = '%i';"
	}
...
#define PLUGIN_AUTHOR "molli"
#define PLUGIN_DESCR "Ban bad ct players"
#define PLUGIN_VERSION "1.800.000"
#define PLUGIN_URL "http://www.supermuroserver.com"
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_ctban", Command_CtBan, ADMFLAG_BAN, "sm_ctban <player> | Bans a player from joining CT.");
	RegAdminCmd("sm_ctunban", Command_CtUnban, ADMFLAG_BAN, "sm_ctunban <player> | Unbans a player from joining CT.");
				...
				if (KvGotoFirstSubKey(kv))
				{
					do
					{
						...
						if (StrEqual(buff, "Database"))
						{
							...
							{
								KvGetString(kv, "database_config", db_config, sizeof(db_config), "default");
							}
						}
	...
	else
	{
		...
		if (g_iDatabaseId[targets[0]] == -1)
		{
			...
			if (GetClientTeam(targets[0]) == g_c_banned_team && GetConVarInt(g_hCvEnabled))
			{
				...
				PrintToChat(targets[0], "%c[CTBan]%c You've been banned from playing on that team!", GREEN, LIGHTGREEN);
			}
...
PrintRejectionMessage(client)
{
	PrintToChat(client, "%c[CTBan]%c You're banned from joining that team!", GREEN, LIGHTGREEN);
	for (new i = 1; i <= MaxClients; i++)
	{
		if (i != client && IsClientConnected(i) && IsClientInGame(i))
		{
			PrintToChat(i, "%c[CTBan]%c %N tried to join CT, but was rejected", GREEN, LIGHTGREEN, client);
		}
	}
}
